...
public class MinusPlugin extends AbstractBinaryOperationPlugin {
    /**
     * Compute the difference of two time series
     *
     * @param ts1 the time series
     * @param ts2 the other time series
     * @return the difference of the two time series
     */
    @Override
    public TimeSeries compute(TimeSeries ts1, TimeSeries ts2) {
        BinaryDoubleOperation minus = (a, b) -> a - b;
        return arithmetic(ts1, ts2, minus, getOpName());
    }
    ...
    public JMenuItem getMenuItem(Framework framework, JFrame frame) {
        return getMenuItemHelper(framework, frame, "Difference");
    }
}
